...
export default function CustomerStatementReport({ customers = [] }) {
  ...
  const params = filter.period === 'range'
    ? { period: 'range', from: filter.from, to: filter.to }
    : filter.period === 'yearly' ? { period: 'yearly', year: filter.year }
  ...
  const tableRows = (data?.entries ?? []).map((e) => ({
    Date: fmtDate(e.date), Type: e.type, Reference: e.reference,
    Description: e.description, Amount: e.amount, Balance: e.runningBalance, Status: e.status,
  }));
  ...
  async function handleSend() {
    ...
    } catch { await navigator.clipboard.writeText(html).catch(() => {}); }
  ...
  return (
    <ReportShell
      ...
      tableRows={tableRows}
      extraActions={
        <button type="button" onClick={handleSend} disabled={!data}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={label} className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                  ...
                  <p className={`mt-1 text-lg font-bold tabular-nums ${highlight && parseFloat(value) >= 0 ? 'text-emerald-600' : highlight ? 'text-rose-600' : 'text-slate-900'}`}>{fmt(parseFloat(value))}</p>
                </div>
              ))}
            </div>
            <ResponsiveContainer width="100%" height={240}>
              <AreaChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                ...
                <Area dataKey="balance" stroke="#6366f1" fill="#e0e7ff" strokeWidth={2} type="monotone" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ) : <p className="py-10 text-center text-sm text-slate-400">Select a customer to view the statement.</p>
      }
    />
  );
}
