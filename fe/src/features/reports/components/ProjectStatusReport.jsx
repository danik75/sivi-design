...
const STATUS_COLORS = { todo: '#94a3b8', in_progress: '#6366f1', done: '#10b981', cancelled: '#ef4444' };
...
export default function ProjectStatusReport({ customers = [] }) {
  ...
  const statusChart = summary
    ? Object.keys(STATUS_COLORS).map((k) => ({ name: STATUS_LABELS[k], count: summary[k] ?? 0, color: STATUS_COLORS[k] }))
    : [];
  ...
  return (
    <div className="space-y-4">
      ...
      <ReportShell
        ...
        tableHeaders={['Customer','Task','Status','Start Date','End Date','Est. Hours','Complete %']}
        tableRows={tableRows}
        chartContent={
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-3">Status Distribution</p>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={statusChart} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <Bar dataKey="count" radius={[3,3,0,0]} maxBarSize={40}>
                    {statusChart.map((e) => <Cell key={e.name} fill={e.color} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-3">Avg Completion by Customer</p>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={completionChart} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false} />
                  <XAxis dataKey="name" tick={{ fontSize: 10, fill: '#94a3b8' }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} width={36} unit="%" domain={[0,100]} />
                  <Tooltip formatter={(v, n) => n === 'avg' ? [`${v}%`, 'Avg Completion'] : [v, 'Tasks']} />
