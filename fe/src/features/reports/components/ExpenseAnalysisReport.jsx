...
export default function ExpenseAnalysisReport() {
  ...
  const catData = (data?.byCategory ?? []).map((r, i) => ({ name: r.category, value: parseFloat(r.total), color: CHART_COLORS[i % CHART_COLORS.length] }));
  const vendorData = (data?.byVendor ?? []).map((r) => ({ name: r.vendor ?? '(none)', total: parseFloat(r.total) }));
  const custData = (data?.byCustomer ?? []).map((r) => ({ name: r.customerName, total: parseFloat(r.total) }));

  const tableRows = subTab === 'category'
    ? (data?.byCategory ?? []).map((r) => ({ Category: r.category, Total: r.total, '%': r.pct, Count: r.count, Currency: r.currency }))
    : subTab === 'vendor'
    ? (data?.byVendor ?? []).map((r) => ({ Vendor: r.vendor ?? '—', Category: r.category, Total: r.total, Count: r.count, Currency: r.currency }))
    : (data?.byCustomer ?? []).map((r) => ({ Customer: r.customerName, Total: r.total, Count: r.count, Currency: r.currency }));

  const tableHeaders = subTab === 'category' ? ['Category','Total','%','Count','Currency']
    : subTab === 'vendor' ? ['Vendor','Category','Total','Count','Currency']
    ...
    <div className="space-y-4">
      {data && (
        <div className="rounded-xl border border-slate-100 bg-rose-50 px-5 py-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-rose-400">Total Expenses</p>
          <p className="mt-1 text-2xl font-bold text-rose-700 tabular-nums">{fmt(parseFloat(data.totalExpenses))}</p>
        </div>
      )}
      <ReportShell
          ...
          <div className="space-y-4">
            ...
            {subTab === 'vendor' && (
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={vendorData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }} layout="vertical">
