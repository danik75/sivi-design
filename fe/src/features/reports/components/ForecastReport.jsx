  ...
  return (
    <div className="space-y-4">
      {data && (
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: 'Confirmed', value: (data?.confirmed ?? []).reduce((s, r) => s + parseFloat(r.amount), 0), cls: 'text-emerald-600' },
            { label: 'Projected', value: (data?.projected ?? []).reduce((s, r) => s + parseFloat(r.amount), 0), cls: 'text-indigo-600' },
          ].map(({ label, value, cls }) => (
      ...
      <ReportShell
        ...
        tableHeaders={['Month','Customer','Source','Reference','Due Date','Amount','Currency']}
        tableRows={tableRows}
        chartContent={
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey="Confirmed" stackId="a" fill="#10b981" radius={[0,0,0,0]} maxBarSize={40} />
              <Bar dataKey="Projected" stackId="a" fill="#6366f1" radius={[3,3,0,0]} maxBarSize={40} />
            </BarChart>
          </ResponsiveContainer>
        }
      />
